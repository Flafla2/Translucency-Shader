#ifndef UNITY_STANDARD_TRANS_CORE_INCLUDED
#define UNITY_STANDARD_TRANS_CORE_INCLUDED

#include "UnityCG.cginc"
#include "UnityShaderVariables.cginc"
#include "UnityStandardConfig.cginc"
#include "UnityStandardTranslucentInput.cginc"
...
half4 fragForwardAdd (VertexOutputForwardAdd i) : SV_Target
{
	...
	half attenRaw = atten / SHADOW_ATTENUATION(i);
...
void fragDeferred (
	...
	out half4 outDiffuse : SV_Target0,			// RT0: diffuse color (rgb), thickness (a)
	out half4 outSpecSmoothness : SV_Target1,	// RT1: spec albedo (r), translucent distortion (g), translucent power (b), smoothness (a)
	out half4 outNormal : SV_Target2,			// RT2: normal (rgb), Translucent? (low precision a) 
	out half4 outEmission : SV_Target3			// RT3: emission (rgb), used later (a)
)
{
	#if (SHADER_TARGET < 30)
		outDiffuse = 1;
	...
	half thickness = Thickness(i.tex.xy);

	outDiffuse = half4(s.diffColor, thickness);
	outSpecSmoothness = half4(s.specColor.r, _TranslucentDistortion, _TranslucentPower/10, s.oneMinusRoughness);
	outNormal = half4(s.normalWorld*0.5+0.5,0);
